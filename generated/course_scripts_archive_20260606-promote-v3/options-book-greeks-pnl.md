...
**Intuition _[reformule]_.** Un book options arrive avec delta/gamma/vega/theta agrege. L'objectif de cette lecon est precisement: Identifier les sensibilites dominantes du book.

**Ce que disent les sources** _[extrait]_. « For example Mello and Neuhaus (1998)
...
**Le point cle: Delta par %, gamma par %^2, vega par vol point.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Verifier unites et signe. Livrable attendu: Risk snapshot - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.

**Piege frequent.** confondre une sensibilite 'par 1%' avec 'par 0.01': respecter strictement les unites.

### Lecon 2 - P&L attribution

**Intuition _[reformule]_.** Spot baisse, vol monte, un jour passe. L'objectif de cette lecon est precisement: Calculer P&L sous scenario spot/vol/time.

**Ce que disent les sources** _[extrait]_. « Figlewski (1998): ‘‘The Information Content of Implied Volatility’’ The
...
**Le point cle: Taylor P&L delta-gamma-vega-theta.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Calculer chaque bloc et le total. Livrable attendu: Attribution table - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.
...
**Intuition _[reformule]_.** Le book est short gamma et long vega. L'objectif de cette lecon est precisement: Proposer une couverture avec residual risk visible.

**Ce que disent les sources** _[extrait]_. « Theta is greater (in absolute value) for short-term ATM options, so statement
d. is incorrect. » [S7]

**Le point cle: Delta hedge, convexity hedge, vega hedge.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Choisir action et trigger. Livrable attendu: Hedge memo - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.
...
**Intuition _[reformule]_.** Le P&L explique doit tenir en 8 lignes. L'objectif de cette lecon est precisement: Ecrire un message utile a trader et risk manager.
...
**Le point cle: Dominant risk, residual risk, monitoring.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Rediger la note. Livrable attendu: Desk risk note - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.
