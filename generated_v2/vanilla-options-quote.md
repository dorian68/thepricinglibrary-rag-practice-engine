...
**Intuition _[reformule]_.** Sales demande un prix indicatif sur un call europeen avant envoi client. L'objectif de cette lecon est precisement: Transformer une demande de quote en inputs propres: spot, strike, maturite, taux, dividendes et vol.

**Ce que disent les sources** _[extrait]_. « The peak vega on a vanilla option reduces over time. Intuitively,
...
optionality and time value is maximized. » [S1]

**Le point cle: Moneyness, forward, discounting, convention de maturite.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Construire le ticket et identifier les donnees manquantes. Livrable attendu: Quote ticket controle - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.

**Piege frequent.** confondre une sensibilite 'par 1%' avec 'par 0.01': respecter strictement les unites.

### Lecon 2 - Prix Black-Scholes

**Intuition _[reformule]_.** Le desk veut un prix defendable et reproductible. L'objectif de cette lecon est precisement: Calculer call et put avec substitutions visibles et unite de premium.

**Ce que disent les sources** _[extrait]_. « Mathematical description of option valuation goes at least back more than
100 years to the now so famous Bachelier (1900) paper, that was based on his doctoral thesis
defended on March 19, 1900. Bachelier assumed a normal distribution for the asset price. » [S4]

**Le point cle: d1/d2, prix call/put, dividend yield.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Calculer le prix et verifier intrinsic/time value. Livrable attendu: Pricing sheet - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.
...
**Intuition _[reformule]_.** Le put mid ne colle pas avec le call mid et le forward. L'objectif de cette lecon est precisement: Detecter une incoherence de quote avant de la transmettre.

**Ce que disent les sources** _[extrait]_. « American Vanilla Pricing and Greeks
...
**Le point cle: C - P = forward discounté moins strike discounté.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Mesurer le parity gap et conclure quote/hold/reject. Livrable attendu: Parity control - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.
...
**Intuition _[reformule]_.** Le client augmente la taille et le trader demande le hedge initial. L'objectif de cette lecon est precisement: Convertir delta et vega en risque concret pour le trader.

**Ce que disent les sources** _[extrait]_. « CCY2 premium,
265–267
...
**Le point cle: Delta hedge, vega per vol point, sign convention.** C'est la notion qui transforme la situation en calcul exploitable. Reliez-la a une intuition de signe ou d'ordre de grandeur avant d'appliquer une formule.

**Mise en pratique _[genere]_.** Calculer hedge shares et sensibilite vol. Livrable attendu: Risk add-on note - un document court contenant le calcul central, une phrase d'interpretation marche et une limite du modele.
